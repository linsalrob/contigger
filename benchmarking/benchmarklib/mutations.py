...
def substitute(seq, count, seed):
    ...
    for p in pos:
        ...
        new = choices[rng.randrange(3)]
        chars[p] = new
        rows.append((p, old, new))
    return "".join(chars), rows


def insert(seq, position, bases):
    return seq[:position] + bases + seq[position:]


def delete(seq, position, length):
    return seq[:position] + seq[position + length :]
