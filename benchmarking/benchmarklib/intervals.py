...
def extract(seq, start, end, strand="+", circular=False):
    ...
    if circular:
        if end - start > n:
            raise ValueError("interval longer than circle")
        part = (seq + seq)[start % n : start % n + end - start]
    else:
        if end > n:
            raise ValueError("interval outside reference")
        part = seq[start:end]
    if strand == "-":
        return reverse_complement(part)
    if strand != "+":
...
def merge_suffix_prefix(left, right, overlap):
    ...
    if left[-overlap:] != right[:overlap]:
        raise ValueError("overlap sequences differ")
