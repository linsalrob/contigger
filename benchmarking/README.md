...
All-vs-all PAF generation uses `--secondary=yes -p 0 -N 100`. The zero secondary-to-primary score ratio is intentional: self hits must not suppress shorter constructed terminal-overlap truth, and the high secondary cap retains repeat evidence for conservative ambiguity classification.
