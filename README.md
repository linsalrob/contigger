...
The deterministic `map-ont` baseline at a 20 bp continuous spanning flank detects 13 of 15 true junctions in every sample and misses `end_tolerance_49` and `end_tolerance_50` in every sample. Quantitative results are sample-scoped: six of 45 true sample-cases are missed. None of the checked-in targeted FASTQs contains a read remapped to any of the four artificial references, so all 12 artificial sample-cases are explicitly untestable and a false-support baseline is **not established**. Zero observed false support must not be interpreted as validated negative performance. The baseline retains per-sample counts, spanning-name digests, provisional-reference checksums, dataset/tool versions, and truth checksums without volatile temporary paths. These results do not establish a reviewed ONT support threshold: relevant artificial-case reads plus mapping-quality, duplicate, flank, and minimum-read policies still require broader truth sets. Consensus and variation policies also remain unimplemented.
