...
Junction truth or support-policy changes must run `pytest tests/test_junction_benchmark.py` and explain every change from `benchmarks/pseudomonas_junction_truth_baseline.json`. Construction-time source-read counts are not remapping observations. New policies remain unreviewed and deferred until their exact technology, alignment configuration, false-support behavior, and missed-support behavior have a checked-in baseline.

Targeted-remapping benchmark changes must reproduce `benchmarks/pseudomonas_junction_remapping_baseline.json` with `contigger benchmark-junction-remapping --dataset test_data --preset map-ont`. This external-tool run is a development baseline, not an ordinary unit test; CI consumes the checked-in deterministic report without invoking minimap2.
Do not count an artificial sample-case as a successful negative unless relevant reads were remapped, and never pool quantitative read counts across samples.
