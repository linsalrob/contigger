...
`load_junction_policy_candidate_baseline()` strictly parses the candidate threshold artifact, rejects duplicate/unknown fields and duplicate thresholds, and can verify its exact SHA-256. `validate_junction_policy_review()` is the pairing gate: an approved policy review must reference reviewed truth metadata with an established false-support baseline and must match the supplied candidate-baseline digest. No current checked-in metadata is reviewed.
