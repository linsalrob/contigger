...
| Candidates | `--kmer-size`, `--window-size`, `--min-shared-minimisers`, `--max-minimiser-frequency`, `--max-seed-pair-observations`, `--max-candidate-pairs` | Candidate settings control recall and runtime, not merge authorization. Leave defaults alone initially. |
